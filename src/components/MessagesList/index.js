...
const MessagesList = ({ ticket, ticketId, isGroup }) => {
  ...
  function getContactData(vcard) {
    const lines = vcard.split(/\r?\n/); // Dividir o vCard em linhas
    ...
    lines.forEach(line => {
      if (line.startsWith("FN:")) {
          contactName = line.substring(3); // Extrair o nome após "FN:"
      } else if (line.startsWith("TEL;type=Mobile;waid=")) {
          const match = line.match(/TEL;type=Mobile;waid=(\d+)/);
          if (match && match[1]) {
              contactPhone = match[1]; // Extrair apenas os dígitos do número de telefone
          }
      }
  });
  ...
  const checkMessageMedia = (message) => {
    if (
      ...
      let { contactName, contactPhone } = getContactData(message.body);

      return (
        <>
          <div className={classes.textContentItem} style={{ paddingRight: 6 }}>
            <div
              style={{
                display: "flex",
                flexDirection: "row",
          ...
          <ContactModal
            ...
            initialValues={{ name: contactName, number: contactPhone }}
